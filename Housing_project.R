...
housing_data <- read.csv(
    "/Users/shreyas/Desktop/R-Projects/corrected_boston.csv"
)
...
ggplot(housing_data, aes(x = RM, y = MEDV)) +
    ...
    labs(
        title = "Rooms vs. Median House Value",
...
ggplot(housing_data, aes(x = factor(CHAS), y = MEDV)) +
    ...
    labs(
        ...
        x = "Proximity to Charles River (1 = Near, 0 = Far)",
        y = "Median Value ($1000)"
    )
...
write.csv(
    housing_data,
    "/Users/shreyas/Desktop/R-Projects/housing_predictions.csv",
    row.names = FALSE
)
...
ggplot(housing_data, aes(x = MEDV, y = Predicted_MEDV)) +
    geom_point(color = "blue") +
    geom_abline(
        intercept = 0, slope = 1, color = "red", linetype = "dashed"
    ) +
    labs(
        title = "Predicted vs. Actual Median House Value",
        x = "Actual Median Value ($1000)",
        y = "Predicted Median Value ($1000)"
    )
...
write.csv(
    housing_data,
    "/Users/shreyas/Desktop/R-Projects/final_housing_data.csv",
    row.names = FALSE
)
